...
const SITE_COLUMNS: Array<[string, (p: Record<string, NotionProp>, page: Page) => string, string]> = [
  ...
  ['planning_pathway', (p) => label(sel(p['Planning Pathway'])), 'Approval pathway (NSW SSD, VIC DFP / ministerial, council, etc.).'],
  ...
  ['governance_flags', (p) => multi(p['Governance Flags']), 'Analyst flags, e.g. transparency deficit.'],
